...
--3.Lấy ra mã giáo viên tên giáo viên và họ tên người thân tương ứng
SELECT GV.MAGV, HOTEN, NT.TEN FROM dbo.GIAOVIEN AS GV, dbo.NGUOITHAN AS NT

--4.Lấy ra mã giáo viên, tên giáo viên và tên khoa của giáo viên đó làm việc, gợi ý: bộ môn nằm trong khoa
SELECT GV.MAGV, GV.HOTEN, K.TENKHOA FROM dbo.GIAOVIEN AS GV, dbo.KHOA AS K, dbo.BOMON AS BM
...
--Lấy ra giáo viên là nữ là lương > 2000
SELECT * FROM dbo.GIAOVIEN WHERE PHAI = N'Nữ' AND LUONG > 2000

--Lấy ra giáo viên nhỏ hơn 40 tuổi
SELECT * FROM dbo.GIAOVIEN WHERE (YEAR(GETDATE()) -YEAR(NGSINH)) < 40

--Lấy ra họ tên giáo viên, năm sinh và tuổi của giáo viên hỏ hơn 40 tuôi
...
SELECT COUNT(*) FROM dbo.GIAOVIEN AS GV, dbo.NGUOITHAN AS NT
WHERE GV.MAGV = NT.MAGV AND NT.MAGV = '007'

--Lấy ra tên giáo viên và tên đề tài người đó tham gia
SELECT GV.HOTEN, DT.TENDT FROM dbo.DETAI AS DT, dbo.GIAOVIEN AS GV, dbo.THAMGIADT AS TG
WHERE GV.MAGV = TG.MAGV AND TG.MADT = DT.MADT

--Lấy ra tên giáo viên và tên đề tài người đó tham gia nhiều hơn 1 lần
SELECT GV.HOTEN, DT.TENDT FROM dbo.DETAI AS DT, dbo.GIAOVIEN AS GV, dbo.THAMGIADT AS TG
WHERE GV.MAGV = TG.MAGV AND TG.MADT = DT.MADT

--Xuất ra thông tin giáo viên và giáo viên quản lý chủ nhiệm của người đó
SELECT GV1.HOTEN, GV2.HOTEN FROM dbo.GIAOVIEN AS GV1, dbo.GIAOVIEN AS GV2 WHERE GV1.GVQLCM = GV2.MAGV 

--Xuất ra số lượng giáo viên khoa CNTT
SELECT COUNT(*) FROM dbo.BOMON AS BM, dbo.GIAOVIEN AS GV, dbo.KHOA AS K
WHERE BM.MABM = GV.MABM AND BM.MAKHOA = K.MAKHOA AND K.TENKHOA = N'Công nghệ thông tin'
